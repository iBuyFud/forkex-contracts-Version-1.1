...
contract MockForkexTreasury is Ownable {
  using SafeMath for uint256;
  ...
  address public sFORK;
  uint256 public sFORKQueue; // Delays change to sFORK address
  ...
  constructor(
    ...
    // isLiquidityToken[ _FORKDAI ] = true;
    // liquidityTokens.push( _FORKDAI );

    blocksNeededForQueue = _blocksNeededForQueue;
  }

  /**
        @notice allow approved address to deposit an asset for FORK
        @param _amount uint
  ...
  function deposit(
    ...
    (_token, _amount);
    // mint FORK needed and store amount of rewards for distribution
    send_ = value.sub(_profit);
    IERC20Mintable(FORK).mint(msg.sender, send_);
  ...
  /**
        @notice allow approved address to burn FORK for reserves
        @param _amount uint
        @param _token address
     */
  function withdraw(uint256 _amount, address _token) external {
    ...
    uint256 value = valueOfToken(_token, _amount);
    IFORKERC20(FORK).burnFrom(msg.sender, value);
  ...
  function incurDebt(uint256 _amount, address _token) external {
    ...
    uint256 maximumDebt = IERC20(sFORK).balanceOf(msg.sender); // Can only borrow against sFORK held
  ...
  /**
        @notice allow approved address to repay borrowed reserves with FORK
        @param _amount uint
     */
  function repayDebtWithFORK(uint256 _amount) external {
    require(isDebtor[msg.sender], "Not approved");

    IFORKERC20(FORK).burnFrom(msg.sender, _amount);

    debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
    totalDebt = totalDebt.sub(_amount);

    emit RepayDebt(msg.sender, FORK, _amount, _amount);
  }
  ...
  function mintRewards(address _recipient, uint256 _amount) external {
    ...
    IERC20Mintable(FORK).mint(_recipient, _amount);
  ...
  function excessReserves() public view returns (uint256) {
    return totalReserves.sub(IERC20(FORK).totalSupply().sub(totalDebt));
  }
  ...
  /**
        @notice returns FORK valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
  ...
  {
    if (isReserveToken[_token]) {
      // convert amount to match FORK decimals
      value_ = _amount.mul(10**IERC20(FORK).decimals()).div(
    ...
    returns (bool)
  {
    ...
    } else if (_managing == MANAGING.SFORK) {
      // 9
      sFORKQueue = block.number.add(blocksNeededForQueue);
    } else return false;
